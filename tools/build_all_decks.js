...
const UNITS = {
  launch:      { primary:'0E1C42', accent:'55C8E8', emoji:'🌟', virtue:'Curiosity',    book:'Launch: Reading Routines' },
  flatStanley: { primary:'0E1C42', accent:'55C8E8', emoji:'⭐', virtue:'Courage',      book:'Flat Stanley' },
  nonfiction:  { primary:'1A4A2E', accent:'5CB85C', emoji:'🗺️', virtue:'Diligence',    book:'Nonfiction: Maps, Mail & Places' },
  despereaux:  { primary:'3D1A6B', accent:'8B6FC0', emoji:'🐭', virtue:'Courage',      book:'The Tale of Despereaux' },
  whippingBoy: { primary:'2A0A4A', accent:'B8860B', emoji:'👑', virtue:'Perseverance', book:'The Whipping Boy' },
  med:         { primary:'0E1C42', accent:'55C8E8', emoji:'📖', virtue:'Wisdom',       book:'Medieval Nonfiction' },
  lww:         { primary:'1A3A5C', accent:'4A90D9', emoji:'🦁', virtue:'Loyalty',      book:'The Lion, the Witch and the Wardrobe' },
  extension:   { primary:'0E1C42', accent:'34B76F', emoji:'🎉', virtue:'Gratitude',    book:'Year-End Synthesis' },
};
let NC = UNITS.despereaux;
...
function titleSlide(pres,d){
  ...
  s.addText(NC.book,{x:0.5,y:1.85,w:9,h:0.62,fontSize:NC.book.length>28?26:34,bold:true,fontFace:F,color:C.white,align:'center',valign:'middle',margin:0});
...
function dolModelSlide(pres,d){
  if(!d.dol_model && !(d.model_hints||[]).length) return;
  const s=cSlide(pres,'✏️  Daily Oral Language',C.cyan);
  // When the lesson marks up individual words with hints, the model "sentence"
  // is only those words strung together and reads as broken English — show the
  // points instead of a quotation.
  const hints=(d.model_hints||[]);
  lbl(s,hints.length?'KEY GRAMMAR POINTS IN THE MODEL':'CORRECT MODEL — STUDY WHAT IS RIGHT',0.4,0.88,C.cyan);
  let y=1.2;
  if(!hints.length && d.dol_model){ qBox(s,pres,d.dol_model,0.4,1.18,9.2,0.9,C.cyanLight,C.darkGray,C.cyan); y=2.2; }
  hints.slice(0,4).forEach((h,i)=>{ card(s,pres,0.4,y,9.2,0.6,i%2?C.white:C.offWhite);
    s.addText([{text:h.word+'  ',options:{bold:true,color:C.navy}},{text:h.hint,options:{color:C.darkGray,fontSize:13}}],
      {x:0.65,y:y+0.1,w:8.6,h:0.4,fontSize:15,fontFace:F,wrap:true,valign:'middle',margin:0}); y+=0.68; });
  s.addNotes((hints.length?'Model sentence in the lesson: '+(d.dol_model||'')+'\n':'')+'Tap through each highlighted word in the lesson and name what it does right before moving to the fix-it.');
}
function dolSlide(pres,d,show){
  if(!d.dol_fix) return;
  const s=iSlide(pres,'✏️  Daily Oral Language',C.cyan);
  lbl(s,d.dol_fix2?'FIX THESE SENTENCES':'NOW FIX THIS ONE',0.4,0.82,C.cyan);
  ...
  if(show){
    ...
    s.addText(d.dol_fix2?'Rewrite both sentences correctly in your notebook.':'Rewrite the sentence correctly in your notebook.',{x:0.4,y:3.05,w:9.2,h:0.4,fontSize:16,bold:true,fontFace:F,color:C.navy,margin:0});
  }
  s.addNotes(`Corrected: ${d.dol_ans} ${d.dol_ans2||''}\nLook for: ${d.dol_errors}`);
}
function morphSlide(pres,d){
  if(!d.morph_items||!d.morph_items.length) return;
  const s=cSlide(pres,'🌱  Morphology',NC.accent);
  lbl(s,(d.morph_title||'WORD PARTS').toUpperCase(),0.4,0.88,C.purple);
  let y=1.2;
  (d.morph_items||[]).slice(0,4).forEach((it,i)=>{ card(s,pres,0.4,y,9.2,0.68,i%2?'F8F4FF':C.purpleLight);
...
function vocabSlide(pres,d){
  ...
  lbl(s,(d.vocab_title||'VOCABULARY').replace(/^\uD83D\uDD24\s*/,'').replace(/^Vocabulary:?\s*/i,'').toUpperCase()||'VOCABULARY',0.4,0.88,C.cyan);
  let y=1.2;
...
function grammarSlide(pres,d,show){
  ...
  lbl(s,(d.gram_title||'GRAMMAR').toUpperCase(),0.4,0.85,C.gold);
  let y=1.18;
  ...
  s.addText(show?'✅  Answers revealed':(d.gram_task||'Tap the word that fits the rule.'),
    {x:0.4,y:y+0.05,w:9.2,h:0.4,fontSize:15,bold:show,italic:!show,fontFace:F,color:show?C.green:C.gray,margin:0});
  s.addNotes('Answers: '+d.grammar.map(g=>g.answer).filter(Boolean).join(', '));
}
function spellSlide(pres,d,show){
  ...
  lbl(s,(d.spell_title||'SPELLING').toUpperCase(),0.4,0.85,C.navy);
  if(!show){
...
const data=JSON.parse(fs.readFileSync(process.argv[2]||'lesson_all.json','utf8'));
const tmw=JSON.parse(fs.readFileSync(process.argv[3]||'tmw_all.json','utf8'));
let rows=data.lessons||data; if(!Array.isArray(rows)) rows=Object.values(rows);
const outDir='/home/claude/decks/out2'; fs.mkdirSync(outDir,{recursive:true});
(async()=>{
  let n=0;
  for(const d of rows){
